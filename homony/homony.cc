...
#include "homony_predictor.h"
#include "flexmingen.h"
#include "helper_function.h"

//Homony predictors for demand and prefetch requests
// Homony对需求和预取请求的预测器
Homony_Predictor* predictor_demand;    // 2K条目，每个条目5位计数器
Homony_Predictor* predictor_prefetch;  //2K entries, 5-bit counter per each entry
...
{
    cout << "Initialize Homony replacement policy state" << endl;
    ...
    predictor_prefetch = new Homony_Predictor();
    predictor_demand = new Homony_Predictor();

    cout << "Finished initializing Homony replacement policy state" << endl;
}
...
void UpdateReplacementState (uint32_t cpu, uint32_t set, uint32_t way, uint64_t paddr, uint64_t PC, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
    ...
    if(SAMPLED_SET(set)){
        ...
        if((type != PREFETCH) && (cache_history_sampler[sample_set].find(sample_tag) != cache_history_sampler[sample_set].end())){
            ...
            // 没有发生时间轮回且这一行在cache中
            ret_ic is_cache_ret = flexmingen_occup_vector[set].is_cache(currentVal, previousVal, type, led, cache_history_sampler[sample_set][sample_tag].PCval);
            bool is_cache = is_cache_ret.cache;
            bool is_reset = is_cache_ret.is_reset;
            if(!isWrap && is_cache){
                demand_hit++;
            ...
            //没命中,负训练
                ...
                if(is_reset){
                    std::set<uint64_t> reset_pcvals = is_cache_ret.PC_vals;
                    if(reset_pcvals.size() > 0){
                        for(std::set<uint64_t>::iterator it = reset_pcvals.begin(); it != reset_pcvals.end(); it++){
                            predictor_demand->increase(*it);
                            predictor_demand->increase(*it);
                        }
                    }
                }
    ...
    //Retrieve Homony's prediction for line
